...
func TestStringQuery(t *testing.T) {
	...
	col, colErr := NewCollection(internalTesting.Path)
	if colErr != nil {
		t.Errorf("can't make the collection object: %s", colErr.Error())
		return
	}
	col.SetIndex("streetName", index.StringIndexType, selector)
