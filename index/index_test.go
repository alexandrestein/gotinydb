...
func TestStringQuery(t *testing.T) {
	...
	listOfTests := []struct {
	...
	}{
		{
			...
			query:          buildTestQuery(1, false, query.NewAction().Do(query.Equal).CompareTo("North street")),
			expectedResult: []string{"S_North_1"},
		}, {
			name:           "Get Equal with limit 5",
			query:          buildTestQuery(5, false, query.NewAction().Do(query.Equal).CompareTo("North street")),
			expectedResult: []string{"S_North_1", "S_North_6", "S_North_11", "S_North_16", "S_North_21"},
		}, {
			name:           "Get Equal with limit 5 and reverted",
			query:          buildTestQuery(5, true, query.NewAction().Do(query.Equal).CompareTo("North street")),
			expectedResult: []string{"S_North_21", "S_North_16", "S_North_11", "S_North_6", "S_North_1"},
		}, {
			name:           "Get Greater and Equal - limit 15",
			query:          buildTestQuery(15, false, query.NewAction().Do(query.Greater).CompareTo("East street")),
			...
			query:          buildTestQuery(10, false, query.NewAction().Do(query.Greater).CompareTo("East street"), query.NewAction().Do(query.NotEqual)),
			...
			query:          buildTestQuery(15, false, query.NewAction().Do(query.Less).CompareTo("West street")),
			...
			query:          buildTestQuery(10, false, query.NewAction().Do(query.Less).CompareTo("West street"), query.NewAction().Do(query.NotEqual)),
			...
			query:          buildTestQuery(5, false, query.NewAction().Do(query.Greater).CompareTo("Z")),
			expectedResult: []string{},
		}, {
			name:           "Less than first",
			query:          buildTestQuery(5, false, query.NewAction().Do(query.Less).CompareTo("A")),
			expectedResult: []string{},
		}, {
			name:           "Greater from start",
			query:          buildTestQuery(5, false, query.NewAction().Do(query.Greater).CompareTo("A")),
			expectedResult: []string{"S_East_4", "S_East_9", "S_East_14", "S_East_19", "S_East_24"},
		}, {
			name:           "Less from end",
			query:          buildTestQuery(5, false, query.NewAction().Do(query.Less).CompareTo("Z")),
